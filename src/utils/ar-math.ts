...
export const calculateSkinTone = (ctx: CanvasRenderingContext2D, landmarks: LandmarkPoint[]) => {
...
export const getBodyMeasurements = (landmarks: LandmarkPoint[]): BodyMeasurementResult | null => {
    // Pose Landmarks
    ...
    const confidence = Math.max(
        ...
        Math.min(
            0.99,
            shoulderWidth > 0.08 && shoulderWidth < 0.45 ? 0.82 : 0.55
        )
    );
...
export const stabilityScore = (samples: number[]) => {
